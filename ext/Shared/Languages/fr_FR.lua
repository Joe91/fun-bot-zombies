...
Language:add(code, "Zombies Drop Nades", "Les zombies lâchent Nades")
Language:add(code, "Zombies drop randomly nades", "Les zombies tombent au hasard")
Language:add(code, "Use Zombie Classes", "Utiliser des classes de zombies")
Language:add(code, "Zombie classes behave different", "Les classes de zombies se comportent différemment")
Language:add(code, "Bot Headshot DamageMultiplier", "Multiplicateur de dégâts par tir à la tête du robot")
Language:add(code, "Bot Explosion DamageMultiplier", "Multiplicateur de dégâts d'explosion de robot")
Language:add(code, "Damage multiplier for explosions ", "Multiplicateur de dégâts pour les explosions")
Language:add(code, "Increment Ammo drop per wave", "Augmenter la chute de munitions par vague")
Language:add(code, "Zombies drop more ammo each wave", "Les zombies lâchent plus de munitions à chaque vague")
Language:add(code, "Increment Nade drop per wave", "Incrémenter la baisse de Nade par vague")
Language:add(code, "Zombies drop more nades each wave", "Les zombies lâchent plus de nades à chaque vague")
Language:add(code, "Max Shoot-Distance", "Distance de prise de vue maximale")
Language:add(code, "Meters before bots will start shooting at players", "Des mètres avant que les robots ne commencent à tirer sur les joueurs")
Language:add(code, "Max Shoot-Height", "Hauteur de prise de vue maximale")
Language:add(code, "Maximum height when a bot is close. Will scale over distance and will be 0 at the MaxShootDistance", "Hauteur maximale lorsqu'un bot est proche. S'adaptera à la distance et sera égal à 0 à MaxShootDistance")
