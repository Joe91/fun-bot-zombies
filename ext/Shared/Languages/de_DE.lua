...
Language:add(code, "Zombies Drop Nades", "Zombies lassen Nades fallen")
Language:add(code, "Zombies drop randomly nades", "Zombies lassen zufällig Nades fallen")
Language:add(code, "Use Zombie Classes", "Verwenden Sie Zombie-Klassen")
Language:add(code, "Zombie classes behave different", "Zombie-Klassen verhalten sich anders")
Language:add(code, "Bot Headshot DamageMultiplier", "Bot-Kopfschuss-Schadensmultiplikator")
Language:add(code, "Bot Explosion DamageMultiplier", "Bot-Explosions-Schadensmultiplikator")
Language:add(code, "Damage multiplier for explosions ", "Schadensmultiplikator für Explosionen")
Language:add(code, "Increment Ammo drop per wave", "Erhöhen Sie den Munitionsabfall pro Welle")
Language:add(code, "Zombies drop more ammo each wave", "Zombies lassen mit jeder Welle mehr Munition fallen")
Language:add(code, "Increment Nade drop per wave", "Erhöhen Sie den Nade-Abfall pro Welle")
Language:add(code, "Zombies drop more nades each wave", "Zombies lassen mit jeder Welle mehr Nades fallen")
Language:add(code, "Max Shoot-Distance", "Maximale Schussentfernung")
Language:add(code, "Meters before bots will start shooting at players", "Meter bevor Bots anfangen, auf Spieler zu schießen")
Language:add(code, "Max Shoot-Height", "Maximale Schusshöhe")
Language:add(code, "Maximum height when a bot is close. Will scale over distance and will be 0 at the MaxShootDistance", "Maximale Höhe, wenn ein Bot in der Nähe ist. Wird über die Entfernung skaliert und beträgt bei MaxShootDistance 0")
