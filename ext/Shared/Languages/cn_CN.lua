...
Language:add(code, "Zombies Drop Nades", "僵尸掉落裸体")
Language:add(code, "Zombies drop randomly nades", "僵尸随机掉落裸体")
Language:add(code, "Use Zombie Classes", "使用僵尸类")
Language:add(code, "Zombie classes behave different", "僵尸类别的行为不同")
Language:add(code, "Bot Headshot DamageMultiplier", "机器人爆头伤害倍数")
Language:add(code, "Bot Explosion DamageMultiplier", "机器人爆炸伤害倍数")
Language:add(code, "Damage multiplier for explosions ", "爆炸伤害倍数")
Language:add(code, "Increment Ammo drop per wave", "每波增加弹药掉落")
Language:add(code, "Zombies drop more ammo each wave", "僵尸每波都会掉落更多弹药")
Language:add(code, "Increment Nade drop per wave", "每波增加 Nade 掉落量")
Language:add(code, "Zombies drop more nades each wave", "僵尸每一波都会掉落更多的肉")
Language:add(code, "Max Shoot-Distance", "最大拍摄距离")
Language:add(code, "Meters before bots will start shooting at players", "机器人开始向玩家射击前几米")
Language:add(code, "Max Shoot-Height", "最大射击高度")
Language:add(code, "Maximum height when a bot is close. Will scale over distance and will be 0 at the MaxShootDistance", "机器人靠近时的最大高度。将随距离缩放，并且 MaxShootDistance 处为 0")
