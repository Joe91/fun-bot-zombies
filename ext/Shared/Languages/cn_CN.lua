...
Language:add(code, "Zombies Drop Nades", "僵尸掉落 Nades")
Language:add(code, "Zombies drop randomly nades", "僵尸随机掉落 nades")
Language:add(code, "Use Zombie Classes", "使用僵尸类")
Language:add(code, "Zombie classes behave different", "僵尸类表现不同")
Language:add(code, "Bot Headshot DamageMultiplier", "机器人爆头伤害倍增器")
Language:add(code, "Bot Explosion DamageMultiplier", "机器人爆炸伤害倍增器")
Language:add(code, "Damage multiplier for explosions ", "爆炸的伤害乘数")
Language:add(code, "Max Shoot-Distance", "最大射程")
Language:add(code, "Meters before bots will start shooting at players", "机器人开始向玩家射击前几米")
Language:add(code, "Max Shoot-Height", "最大射高")
Language:add(code, "Maximum height when a bot is close. Will scale over distance and will be 0 at the MaxShootDistance", "机器人靠近时的最大高度。将随距离缩放并且在 MaxShootDistance 处为 0")
