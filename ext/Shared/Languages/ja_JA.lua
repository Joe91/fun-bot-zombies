...
Language:add(code, "Zombie classes behave different", "ゾンビクラスの動作が異なる")
Language:add(code, "Bot Headshot DamageMultiplier", "ボットのヘッドショットダメージ倍率")
Language:add(code, "Bot Explosion DamageMultiplier", "ボットの爆発ダメージ倍率")
Language:add(code, "Damage multiplier for explosions ", "爆発のダメージ倍率")
Language:add(code, "Increment Ammo drop per wave", "ウェーブごとに弾薬ドロップを増加")
Language:add(code, "Zombies drop more ammo each wave", "ゾンビはウェーブごとにより多くの弾薬をドロップします")
Language:add(code, "Increment Nade drop per wave", "ウェーブごとにナデドロップを増加")
Language:add(code, "Zombies drop more nades each wave", "ゾンビはウェーブごとにより多くのナデをドロップします")
Language:add(code, "Max Shoot-Distance", "最大射撃距離")
